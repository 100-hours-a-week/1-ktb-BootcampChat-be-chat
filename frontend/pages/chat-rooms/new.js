...
function NewChatRoom() {
  ...
  const joinRoom = async (roomId, password) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_CHAT_URL}/api/rooms/${roomId}/join`, {
        method: 'POST',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 채팅방 생성
      const response = await fetch(`${process.env.NEXT_PUBLIC_CHAT_URL}/api/rooms`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': currentUser.token,
